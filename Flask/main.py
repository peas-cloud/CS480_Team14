...
table_names = ["A.IUCR", "A.FBI", "B.LOCATION", "A.DISTRICT", "A.SEASON", "B.DIVISION", 
               "B.GEO_COORDINATES", "B.POLICE_SENTIMENT_SCORE", '"C".CRIME_INCIDENT']
...
query_list = {
    "signup": """INSERT INTO "C".Users (firstname, lastname, password, typeofUser, email) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')""",

    "login": """SELECT * FROM "C".Users WHERE EMAIL = '{0}' AND PASSWORD = '{1}'""",
...
FROM (
  ...
  FROM "C".Crime_Incident 
) C
INNER JOIN "A".IUCR I on I.Iucr_Code = C.Iucr_Code 
INNER JOIN "A".Season S on S.Month = C.Month
...
FROM (
    ...
    (
        ...
        FROM "C".Crime_Incident CI
        INNER JOIN "A".IUCR I on I.Iucr_Code = CI.Iucr_Code
        INNER JOIN "B".LOCATION L ON L.Loc_Id = CI.Loc_Id
        INNER JOIN "B".DIVISION D ON L.Division_Id = D.Division_Id
        INNER JOIN "B".GEO_COORDINATES G on G.Geo_Id = L.Geo_Id 
        INNER JOIN (
            SELECT District_Id, District_Name, I.Latitude as District_Latitude, I.Longitude as District_Longitude 
            FROM "A".DISTRICT H
            INNER JOIN "B".GEO_COORDINATES I on H.Geo_Id = I.Geo_Id
...
FROM (
  ...
  FROM (
    ...
    FROM "C".Crime_Incident C
    INNER JOIN "B".Location L on C.Loc_Id = L.Loc_Id
  ) A
...
FROM (
  ...
  FROM (
    ...
    FROM "B".police_sentiment_score ) A
) B
